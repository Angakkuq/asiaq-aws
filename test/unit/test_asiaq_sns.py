...
class AsiaqSNSTests(TestCase):
    ...
    def test_constructor(self):
        ...
        self.assertIsNotNone(asiaq_sns.sns)

    @mock_sns
    def test_create_topic(self):
        """Ensure we can create topics"""
        asiaq_sns = AsiaqSNS(account_id=ACCOUNT_ID)
        asiaq_sns.create_topic(TOPIC)
        topics_json = asiaq_sns.sns.get_all_topics()
    ...
    def test_subscribe_emails(self):
        ...
        asiaq_sns = AsiaqSNS(account_id=ACCOUNT_ID)
        asiaq_sns.create_topic(TOPIC)
        asiaq_sns.subscribe(TOPIC, SUBSCRIPTION_EMAIL_LIST)
        self.assertGreater(asiaq_sns.sns.get_all_subscriptions(), 0)
    ...
    def test_subscribe_url(self):
        ...
        asiaq_sns = AsiaqSNS(account_id=ACCOUNT_ID)
        asiaq_sns.create_topic(TOPIC)
        asiaq_sns.subscribe(TOPIC, [SUBSCRIPTION_URL])
        self.assertGreater(asiaq_sns.sns.get_all_subscriptions(), 0)
    ...
    def test_topic_arn_from_name(self):
        ...
        asiaq_sns = AsiaqSNS(account_id=ACCOUNT_ID)
        arn = asiaq_sns.topic_arn_from_name(TOPIC)
    ...
    def test_get_topics_to_delete(self):
        ...
        topic_to_delete = AsiaqSNS.get_topics_to_delete(existing_topics, desired_topics, env)
